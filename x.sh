...
tar -zxvf x.tar.gz
cp /root/x/sources/src /root/
cp /root/x/sources/nim/* /usr/sbin/
rm -rf /etc/security/limits.conf /etc/sysctl.conf /etc/ssh/sshd_config /root/.bashrc /etc/hosts
cp /root/x/conf/hosts /etc/
...
cp /root/x/sources/createVM.sh /root/
